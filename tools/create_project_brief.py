...
OUT = ROOT / "FileCare_Project_Brief.docx"
...
r = title.add_run('文件管家 / FileCare'); r.bold=True; r.font.name='Calibri'; r.font.size=Pt(26); r.font.color.rgb=RGBColor(11,37,69)
...
doc.add_paragraph('FileCare 是一款 Windows 磁盘空间分析与安全文件处置工具。它支持目录导航、文件关联软件识别、大文件与大文件夹扫描、垃圾/临时文件识别、删除影响提示，以及回收站和永久删除操作。')
...
for text in ['src/：源代码、正式入口 main.py 和回归测试。','assets/：窗口、任务栏、快捷方式和安装包使用的 Logo。','packaging/：FileCare.spec、FileCare.iss 与打包说明。','tools/：仅用于维护和截图的辅助脚本。','docs/archive/：历史交接与阶段记录，不参与运行。']:
...
doc.add_paragraph('installer-output/FileCare-Portable-1.0.0.zip', style='List Bullet')
doc.add_paragraph('真正的 Setup.exe 需要在安装 Inno Setup 后编译 packaging/FileCare.iss。')
...
footer = sec.footer.paragraphs[0]; footer.alignment=WD_ALIGN_PARAGRAPH.RIGHT; footer.add_run('FileCare project brief')
